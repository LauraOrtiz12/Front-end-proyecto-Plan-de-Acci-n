...
<template>
    <Card class="w-full">
        ...
        <div class="flex border-b">
            ...
            <label for="tab1" :class="[tab == 2 ? 'border-b-4 border-primary-700 bg-primary-default text-white transition-all' : 'hover:border-b-4 text-secondary-default hover:bg-secondary-200 hover:border-secondary-default hover:border-secpndary-default ', 'tab-label py-2 px-4 cursor-pointer ']" @click="tab = 2">Indicadores de Prespuesto</label>
        </div>
        <div id="content1" :class="[tab == 1 ? '' : 'hidden', 'tab-content p-4']">
            <slot name="t1"/>
        </div>
        <div id="content2" :class="[tab == 2 ? '' : 'hidden', 'tab-content p-4']">
            <slot name="t2"/>
        </div>
